...
function saveAuditRecord() {
  ...
  // 1. Save locally in user history (loading first to avoid overwriting)
  try {
    const saved = localStorage.getItem('unisenai_extension_audit_history');
    const history = saved ? JSON.parse(saved) : [];
    history.push(auditRecord);
    localStorage.setItem('unisenai_extension_audit_history', JSON.stringify(history));
  } catch (e) {
    console.error("Erro ao salvar histórico de auditoria local:", e);
  }
